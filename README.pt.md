...
| `dbPath` | `''` → `$DSH_HOME/dsh-memento/memory.db` | absoluto, ou relativo a `$DSH_HOME`; quando `$DSH_HOME` não é exportado (o padrão no Windows — `dsh web` não devolve o home resolvido ao ambiente), ambos caem para `~/.dsh` |
