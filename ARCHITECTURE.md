...
- **宿主设置面（0.5.15：0.1.7-alpha 的 live-config 契约）**：0.1.7 线删掉整条注册面后，**插件自己的 `Config` 就是设置面**——表单 namespace = profile entry id（`cordis.patch.yml` 的 `id: memento`，导出为 `SETTINGS_ENTRY_ID`），可编辑字段 = 标了 `.volatile()` 的字段（`SHARED_CONFIG_FIELDS` + `panel`；`enabled` 留普通字段，组合面开关不进面板），编辑由 Loader 直接提交进 `apply` 收到的实时引用并发 `loader/volatile-update`（volatile-only 提交**不重挂 fiber**）。`apply` 只声明呈现策略 `settings.configure({ auto: false }, ctx.fiber)`（自带设置页 ⇒ 不生成重复表单），挂在 `ctx.effect` 上持有 disposer——`SettingsForms.configure` 对同一 fiber 重复调用会抛。值来源是**懒读**（每次读重新解引用），启动期字段的差异在 `loader/volatile-update` 上对账：热字段（writePolicy(s)/language/budgets/各 limit/proposals/panel）即时生效；`dbPath`/`auditRetentionDays` 差异重开 store 并关旧库；`retrieval.vector` 差异拆旧装新检索器；`snapshotOrder` 与工具描述是**注册期固定**的面，响亮写 `settings-startup-fields` 审计并要求重载（volatile 编辑不再重挂 fiber，这是与旧线唯一的行为差异）。数值下限同时写进 schema，越界编辑在写入路径即被拒；跨字段规则（writePolicies 键文法 / budgets 形状）schemastery 3.18.3 无法表达（没有 `.check()`），仍由 `validateMemoryConfig` 加载期拒绝、运行期拒后留 `settings-rejected` 审计行。`.volatile()` 是 3.18.3 起的运行期方法而 schema 在模块求值期构建，故经 `liveField()` 能力探测：旧 schemastery 上字段保持普通值，设置面回落到保留的 `installSection` 形状分派（0.1.2-rc.1 / 0.1.5-alpha.1 / 0.1.6-0 线上那是唯一注册面）。任何设置服务缺失（headless）时 live 保持组合值，行为与未接入前一致。
- **设置一级项（零构建）**：`client/client.js` 向 `settings.section` slot 注册 id 为 `dsh-memento` 的一级设置项（宿主设置弹窗左侧菜单即该 slot 账本渲染，无白名单；`label` 即插件名，`order` 用普通值不抢内置位置）。设置句柄两条线各取其一：0.1.7 线 `ctx.configForms.get(SETTINGS_ENTRY_ID)`（`ConfigForm{getSnapshot, subscribe, set, unset, mutate}`），旧线 `ctx.settingsScope.bind({ namespace: 'dsh-memento' })`；两者都**不写进 inject**（对面宿主上那条线的服务永不到场，写进去会让 fiber 永久 PENDING，面板一起消失），缺席时降级为只读卡片。页面内容走统一暂存—保存/放弃/单字段重置语义（对齐宿主 CardForm）；两条句柄在本类用到的面上同形（`{status, value, user, base, writable}` + `subscribe` + `set(field, value)`）。保存落盘后若涉及 `panel.enabled` 则同帧切换本页悬浮按钮显隐（无需刷新）；界面语言跟随所选（含草稿）语言。写入按顶层字段聚合（`set(top, 合并值)`，路径寻址而非整体替换）。悬浮窗开关经 `/api/memento/entries` 响应的 `panel` 字段贯通到面板启动探测——host 与面板同源，不依赖浏览器读 settings。
