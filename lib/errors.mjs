...
export class MemoryError extends Error {
  ...
  /** @returns {{code: string, message: string} & object} 工具结果里的规范错误对象。 */
  toPublic() {
...
export class EntryNotFoundError extends MemoryError {
  /** @param {object} d - {track, scope, match}。 */
  constructor(d) {
    super(
...
export class AmbiguousMatchError extends MemoryError {
  /**
   * @param {object} d - {track, scope, match, candidates, sample}；sample 是截断后的候选文本。
   */
  constructor(d) {
    super(
...
/**
 * 把任意错误转成可进入工具规范 JSON 结果的公开错误对象。
 * @param {unknown} error - 任意抛出值。
 * @returns {{code: string, message: string}} MemoryError 返回其 toPublic()；
 *   其它错误折叠为 INTERNAL 并保留消息（响亮，不静默）。
 */
export function errorToPublic(error) {
  if (error instanceof MemoryError) return error.toPublic()
